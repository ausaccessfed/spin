...
RSpec.feature 'Roles Admin', type: :feature do
  ...
  given!(:assoc) { create(:subject_role, role: role) }
  given!(:api_assoc) { create(:api_subject_role, role: role) }
  given!(:other_subject) { create(:subject) }
  given!(:api_subject) { create(:api_subject) }

  given(:base_path) { '/admin' }
  ...
  scenario 'adding a permission to a role' do
    ...
    attrs = attributes_for(:permission).merge(value: 'x')
