...
RSpec.describe SubjectRolesController, type: :controller do
  let(:user) do
  ...
  context 'get :new' do
    ...
    it { is_expected.to render_template('subject_roles/new') }
  ...
  context 'post :create' do
    def run
      post(:create, base_params.merge(subject_roles: attrs))
    end
  ...
  context 'delete :destroy' do
    ...
    let!(:assoc) { create(:subject_role, role: role) }
    subject { -> { run } }
    it { is_expected.to have_assigned(:role, role) }
